...
static int set_option_value(copt_t* opt, const char * arg, char* val)
{
    ...
    case COPT_FLOAT:
        if (!val || !opt->vptr) return -1;
        value.v_float = strtof(val, &np);
        if (np == val || *np != 0) return -1;
        if (errno == ERANGE) {
            return -1;
        }
        break;
    ...
    case COPT_USHORT:
        *(short*)opt->vptr = value.v_short;
        break;
    case COPT_FLOAT:
        *(float*)opt->vptr = value.v_float;
        break;
    case COPT_DOUBLE:
        *(double*)opt->vptr = value.v_double;
        break;
    case COPT_CHAR:
...
static int sprintf_option_value(const copt_t* const opt, char * const buf)
{
    ...
    case COPT_CHAR:
        ...
        break;
    case COPT_FLOAT:
        ret = sprintf(buf, "%f", *((float*)opt->vptr));
        break;
    case COPT_DOUBLE:
    ...
    case COPT_URL:
        {
            unsigned int n = cint32_hton(*(unsigned int*)opt->vptr);
            sprintf(buf, "%u.%u.%u.%u",
    ...
    case COPT_SOCKADDR:
        {
            ...
            unsigned int n = cint32_hton(p->a);
            sprintf(buf, "%u.%u.%u.%u:%u",
                n >> 24, (n >> 16) & 0xFF, (n >> 8) & 0xFF, n & 0xFF, p->p);
        }
        break;
    }
    return ret;
